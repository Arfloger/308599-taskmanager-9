...
/* 
 Корректно ли для функций по типу getMenuElement делать стрелочные? 
 ...
 const getMenuElement = () => { 
  ...
  Или это избыточность?

*/

const getMenuElement = function() {
  return `
...
const getFilterElement = function() {
    return `
...
const getCardEditElement = function() {
    return `
...
const getLoadMoreElement = function() {
    return `
...
// Как правильно именовать переменные, которые я достаю из разметки?
// К какому классу тега из разметки цепляться: к блоку или к элементу? Н-р: main__control или control?

const menuElement = document.querySelector(`.control`);
const mainElement = document.querySelector(`.main`);

const renderSomeElement = (insertPlase, callback, interation = 1) => {
  if (interation !== 1) {
      for (let i = 0; i < interation; i++) {
        insertPlase.insertAdjacentHTML(`beforeend`, callback());
      }
      return;
  }
  insertPlase.insertAdjacentHTML(`beforeend`, callback());
...
renderSomeElement(menuElement, getMenuElement);
renderSomeElement(mainElement, getSearchElement);
renderSomeElement(mainElement, getFilterElement);
...
renderSomeElement(boardTasksElement, getCardEditElement);
renderSomeElement(boardTasksElement, getCardElement, 3);
renderSomeElement(boardElement, getLoadMoreElement);